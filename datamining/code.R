...
emploi_histo <- cbind(emploi_histo,ACCIDENT_TREND)
...
emploi <- subset(emploi,select = c(COMMUNE_CODE, COMMUNE_NOM.x,PERCENT_EMPLOI_2016,EMPLOI_2016_03_31,TOTALE))
...
#library(RMongo)
#L=lapply(split(couverture,rownames(couverture)),as.list)  
#names(L)=NULL  
#for (i in 1:NROW(L)) {
#  dataJSON = toJSON(L[[i]])  
#  output <- dbInsertDocument(mongo, "couverture", as.character(dataJSON))
#}
...
# 
# library(dplyr)
# datasets <- fread("datasets.csv",encoding = "UTF-8")
# 
# datasets  <-
#   datasets %>%
#   select(title,slug,url,organization,description,frequency,created_at,last_modified,tags)
# 
# m1<-c(1,2,3)
# m2<-c(4,5,6)
# rbind(m1,m2)
...
#gameofcodeCTG
#christophe.bovy@ctg.com
#game0fc0dectg

library(wordcloud)
d <- data.frame(word = c("Transport","Sport","Creche"),freq=c(100,30,22))
png("juciv.png", width=12,height=8, units='in', res=300)
wordcloud(d$word,d$freq,min.freq=2,max.words=100, random.order=FALSE, rot.per=.15)
dev.off()
...
text <- c("je suis fatigué de coder","moi aussi je suis fatigué")
text <- lapply(text,Unaccent)
